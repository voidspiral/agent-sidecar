    ...
    def test_page_and_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("以太网发送 (B/s)", body)
                self.assertIn("以太网接收 B/s", body)
                self.assertNotIn("Eth rx", body)
                self.assertNotIn("eth tx (B/s)", body)
                self.assertIn("相对首个采样点", body)
                self.assertIn("afterDraw", body)
                self.assertIn("anomalyMarkers", body)
                self.assertIn("overlayMarkers", body)
                self.assertIn("suggestedMax", body)
