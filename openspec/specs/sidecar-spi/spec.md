...
Defines the per-node supervisor and the deterministic tool plugin contract used to collect process, SLURM, MPI, node, and host-ethernet evidence without putting an LLM on the sampling path.
