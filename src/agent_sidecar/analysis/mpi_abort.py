...
def _parse_abort(text: str) -> tuple[int | None, int | None]:
    ...
    for line in text.splitlines():
        if "MPI_Abort" not in line and "mpi_abort" not in line.lower():
            # Still allow "called MPI_Abort" style lines already matching.
        ...
        if "MPI_Abort" in line and rank is None:
            ...
            if m2:
                rank = int(m2.group(1))
    # Prefer "rank N called MPI_Abort" even without MPI_Abort filter above.
    if rank is None:
...
def chinese_summary(analysis: dict[str, Any]) -> str:
    ...
    items = [
        ...
        "4. 建议：核对 abort rank 的错误路径；编译到共享路径后复现，例如 "
        "`python3 -m agent_sidecar srun --agent-match=mpi_fault_abort -n3 -- "
    ...
    if hits:
        loc = hits[0]
        items.append(
            f"5. 源码命中：{loc.get('path')}:{loc.get('lineno')} → {loc.get('line', '').strip()}"
        )
    return "\n".join(items)
